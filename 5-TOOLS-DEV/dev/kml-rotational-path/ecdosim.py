...
# NEW: white for the 360° base circles
WHITE = "ffffffff"   # opaque white in aabbggrr KML format
...
# NEW: circle mode (104 = original behaviour, 360 = add white 360° circle under arcs)
circle_mode = 104
...
try:
    with open('input.txt', 'r') as file:
        ...
        # NEW: optional last input line: circle_mode (104 or 360)
        # If missing or invalid, we keep default = 104
        try:
            circle_mode = int(file.readline().strip().split()[0])
            print(f"Using circle span mode: {circle_mode} degrees.")
        except (IndexError, ValueError):
            # silently fall back to default 104
            pass
...
# NEW: helper to optionally draw a 360° white circle at a given radius
def draw_white_circle(center_lat, center_lon, thickness, circle_mode):
    """
    If circle_mode == 360, draw a full 360° white circle at the radius of
    (center_lat, center_lon) around (a, b) with the given thickness.
    """
    if circle_mode != 360:
        return
    # 180° clockwise and 180° counter-clockwise -> full 360°
    full_circle_points = rotate_point(a, b, center_lat, center_lon, 180, 180)
    add_linestring_to_kml(kml, full_circle_points, WHITE, thickness)
...
# MAIN radius (x, y): draw base white circle (if requested), then 3×104° arcs
draw_white_circle(x, y, THICK, circle_mode)
...
for i in range(1, j + 1):
    ...
    # For this radius, draw the 360° white base circle (if enabled)
    draw_white_circle(center1[0], center1[1], THIN, circle_mode)
    ...
    # Base white circle for this radius as well
    draw_white_circle(center2[0], center2[1], THIN, circle_mode)
